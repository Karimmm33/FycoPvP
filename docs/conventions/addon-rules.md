...
## HARD RULE 6 — grep for orphaned references after deleting anything

Lua resolves a deleted `local` to a **nil global**. That parses perfectly and
only explodes at runtime, so no syntax check will catch it.

This shipped broken. `MINE_SIZE` was removed from `Modules/Auras.lua` when icon
sizes became configurable, but one use survived in `MakeGroup`. `THEIR_SIZE +
MINE_SIZE` threw inside `Auras:OnLoad`, and because the module loop had no
pcall, it aborted the whole `PLAYER_LOGIN` handler — every module not yet
reached never loaded, Options never registered its panels, and the addon
vanished from Interface → AddOns entirely. One arithmetic error presented as a
total failure to load.

So after removing or renaming *anything*:
...
`scripts/luacheck.py` has an `undeclared_constants` pass for precisely this: it
flags ALL_CAPS names used but never declared `local` in the same file, skipping
`ns.FIELD` accesses and table-constructor keys. Validate any detector against
the known-bad input before trusting a clean run from it.

`Core.lua` now wraps each `OnLoad` in a pcall and prints which module failed, so
the next one of these costs a single module instead of the whole addon. That is
containment, not a licence to skip the grep.
